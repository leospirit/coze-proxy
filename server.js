...
async function pollConversation(conversationId) {
  ...
  for (let i = 0; i < maxTries; i++) {
    ...
    if (data.code === 0 && Array.isArray(data.data)) {
      const msg = data.data.find(m => m.role === 'assistant' && m.content);
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    console.log('[CREATE]', JSON.stringify(createData).slice(0, 300));

    const conversationId = createData?.data?.conversation_id || createData?.data?.id;
    if (!conversationId) return res.json({ answer: '（无法获取会话ID）' });
